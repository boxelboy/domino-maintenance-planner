 		...
 		onCommitClick: function (StartDate, EndDate) {
 			...
 			contracts.fetch({
 				...
 				success: _.bind(function () {
 					...
 					var pmSize = _.size(this.plannedMaintenance);
 					var loopCount = 0;
					...
					_.each(this.plannedMaintenance.models, _.bind(function (model) {
 						if (model.get('new') == "Yes") {
 							...
 							if (new Date(model.get('start_date')) > new Date(StartDate.split("-").reverse().join("-") + 'T00:00') && new Date(model.get('end_date')) < new Date(EndDate.split("-").reverse().join("-") + 'T23:59')) {
 								...
 								model.save().done(function(savedModel) {
 									var json = savedModel;
 									console.log(json, json.pmid, json.id, savedModel.pmid, savedModel.id);

 									dummyArr.push(savedModel);		// pushing 'scheduled' into array
 				...
 				var oldDate = "";
 				if(_.isArray(this.assetList))
 				{
 					console.log(this.assetList[0]);
 					oldDate = this.assetList[0].has('start_date') ? this.assetList[0].get('start_date') : "";
 				}
 											...
 											function (model) {
 												if (!found) {
 													if (model.get('asset_recordid') == scheduleItem.get('asset_recordid')) {
 														console.log(scheduleItem.get('preferred_start_date'), model.get('asset_recordid'));
