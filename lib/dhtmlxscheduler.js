...
window.dhtmlXScheduler = window.scheduler = {
    ...
    if (_isIE) return t || (t = this.xmlDoc.nodeName ? this.xmlDoc : this.xmlDoc.responseXML), t || dhtmlxError.throwError("LoadXML", "Incorrect XML", [t || this.xmlDoc, this.mainObject]), i && t.setProperty("SelectionNamespaces", "xmlns:xsl='" + i + "'"), "single" == s ? t.selectSingleNode(e) : t.selectNodes(e) || new Array(0);
    var a = t;
    ...
    function() {
        function e(e, t) {
            var s = e.callback;
            i(!1), e.box.parentNode.removeChild(e.box), c = e.box = null, s && s(t)
        }
        ...
        function n(t, i, a) {
            ...
            var r = "";
            if (t.width && (n.style.width = t.width), t.height && (n.style.height = t.height), t.title && (r += '<div class="dhtmlx_popup_title">' + t.title + "</div>"), r += '<div class="dhtmlx_popup_text"><span>' + (t.content ? "" : t.text) + '</span></div><div  class="dhtmlx_popup_controls">', i) {
    ...
    setUpdated: function(e, t, i) {
        if (!this._silent_mode) {
            ...
            a && "updated" == i && (i = a), t ? (this.set_invalid(e, !1), this.updatedRows[s] = e, this.obj.setUserData(e, this.action_param, i), this._in_progress[e] && (this._in_progress[e] = "wait")) : this.is_invalid(e) || (this.updatedRows.splice(s, 1), this.obj.setUserData(e, this.action_param, "")), t || this._clearUpdateFlag(e), this.markRow(e, t, i), t && this.autoUpdate && this.sendData(e)
        }
    },
    ...
    _getAllData: function() {
        for (var e = {}, t = !1, i = 0; i < this.updatedRows.length; i++) {
            var s = this.updatedRows[i];
            this._in_progress[s] || this.is_invalid(s) || this.callEvent("onBeforeUpdate", [s, this.getState(s), this._getRowData(s)]) && (e[s] = this._getRowData(s, s + this.post_delim), t = !0, this._in_progress[s] = (new Date).valueOf())
        }
        return t ? e : null
    },
    ...
    dhtmlxEvent(window, "resize", function() {
        var s = e();
        t(i, s) || (window.clearTimeout(scheduler._resize_timer), scheduler._resize_timer = window.setTimeout(function() {
    ...
    for (var e = this._obj.getElementsByTagName("DIV"), t = 0; t < e.length; t++) {
        ...
        "string" != typeof a && s && !e[t].innerHTML && (a = s.split("_")[0]), a && (e[t].innerHTML = a)
    }
    ...
    if (this._drag_mode) {
        ...
        if (t.force_redraw || this._is_pos_changed(this._drag_pos, t)) {
            var i, s;
            if (this._edit_id != this._drag_id && this._close_not_saved(), this._drag_pos = t, this._drag_pos.has_moved = !0, "create" == this._drag_mode) {
            ...
            if (this._table_view || c.getDate() == u.getDate() && c.getHours() < this.config.last_hour || scheduler._allow_dnd)
                if (o.start_date = u, o.end_date = new Date(s), this.config.update_render) {
    ...
    if (2 != e.button && !this.config.readonly && !this._drag_mode) {
        ...
        if (this._drag_mode) {
            var s = this._locate_event(t);
            this.config["drag_" + this._drag_mode] && this.callEvent("onBeforeDrag", [s, this._drag_mode, e]) ? (this._drag_id = s, this._drag_event = scheduler._lame_clone(this.getEvent(this._drag_id) || {}), this._drag_pos = this._mouse_coords(e), this._drag_pos.start = +new Date) : this._drag_mode = this._drag_id = 0
        }
    ...
    if (!e || 2 != e.button || !scheduler.config.touch) {
        if (this._drag_mode && this._drag_id) {
            ...
            if (this._drag_event._dhx_changed || !this._drag_event.start_date || s.start_date.valueOf() != this._drag_event.start_date.valueOf() || s.end_date.valueOf() != this._drag_event.end_date.valueOf()) {
            ...
            this._drag_pos && this._drag_pos.has_moved && this.render_view_data(), scheduler.callEvent("onDragEnd", [t, i, e])
        }
    ...
    "month" == this._mode && 0 === e && this.config.left_border && (n.className += " dhx_scale_bar_border", t += 1), this.set_xy(n, r, this.xy.scale_height - 2, t, a), n.innerHTML = this.templates[this._mode + "_scale_date"](i, this._mode), s.appendChild(n)
    ...
    if (this.templates[this._mode + "_date"]) {
        ...
        if (a = n = this._table_view ? scheduler.date.week_start(r) : r, this._min_date = a, this._els.dhx_cal_date[0].innerHTML = this.templates[this._mode + "_date"](r, d, this._mode), this._max_date = d, scheduler._render_scales(e, t), this._table_view) this._reset_month_scale(t, r, n);
    ...
    for (var i = new Date(1980, 1, 1, this.config.first_hour, 0, 0), s = 1 * this.config.first_hour; s < this.config.last_hour; s++) {
        var a = document.createElement("DIV");
        a.className = "dhx_scale_hour", a.style.height = this.config.hour_size_px - (this._quirks ? 0 : 1) + "px";
        var n = this.xy.scale_width;
        this.config.left_border && (n -= 1, a.className += " dhx_scale_hour_border"), a.style.width = n + "px", a.innerHTML = scheduler.templates.hour_scale(i), t.appendChild(a), i = this.date.add(i, 1, "hour")
    }
    ...
    for (var _ = 0, c = "<table cellpadding='0' cellspacing='0'>", u = [], l = 0; d > l; l++) {
        ...
        for (var g = Math.max(s(l) - scheduler.xy.month_head_height, 0), f = 0; 7 > f; f++) {
            c += "<td";
            var v = "";
            t > i ? v = "dhx_before" : i >= a ? v = "dhx_after" : i.valueOf() == r.valueOf() && (v = "dhx_now"), this._ignores_detected && this._ignores[f] && (v += " dhx_scale_ignore"), c += " class='" + v + " " + this.templates.month_date_class(i, r) + "' >";
    ...
    var l, h = new Date(1 * e + t * n),
        ...
        c = 0;
    for (i.render ? (c = this._get_date_index(i, a), l = this._get_date_index(i, h)) : l = Math.round(t / 60 / 60 / 1e3 / 24); l * n >= c * n;) {
    ...
    _add_days: function(e, t) {
        ...
        return i.setDate(i.getDate() + t), !e.getHours() && i.getHours() && (i.getDate() < e.getDate() || i.getMonth() < e.getMonth() || i.getFullYear() < e.getFullYear()) && i.setTime(i.getTime() + 36e5 * (24 - i.getHours())), i
    },
    add: function(e, t, i) {
    ...
    var n = e;
    1 != arguments.length && (n = a || {}, n.start_date = e, n.end_date = t, n.text = i, n.id = s), n.id = n.id || scheduler.uid(), n.text = n.text || "", "string" == typeof n.start_date && (n.start_date = this.templates.api_date(n.start_date)), "string" == typeof n.end_date && (n.end_date = this.templates.api_date(n.end_date));
    ...
    if (e = this._table_view ? this._pre_render_events_table(e, t) : this._pre_render_events_line(e, t), this._table_view)
        if (t) this._colsS.heights = s;
            ...
            if (r.rows) {
                ...
                x.style.height = m, x.style.visibility = -1 == a[0] ? "hidden" : "visible", x.className = a[0] ? "dhx_multi_day_icon" : "dhx_multi_day_icon_small", this._dy_shift = (a[0] + 1) * i, this.config.multi_day_height_limit && (this._dy_shift = Math.min(this.config.multi_day_height_limit, this._dy_shift)), a[0] = 0, v != f && (n.style.top = parseInt(n.style.top) + 2 + "px", p.style.overflowY = "auto", x.style.position = "fixed", x.style.top = "", x.style.left = "")
            }
        }
    return e
    ...
    for (var a = 0; a < e.length; a++) {
        ...
        if (n._sday = this._get_event_sday(n), this._ignores[n._sday]) e.splice(a, 1), a--;
            ...
            }(o < this.config.first_hour || l >= this.config.last_hour) && (s.push(n), e[a] = n = this._copy_event(n), o < this.config.first_hour && (n.start_date.setHours(this.config.first_hour), n.start_date.setMinutes(0)), l >= this.config.last_hour && (n.end_date.setMinutes(0), n.end_date.setHours(this.config.last_hour)), n.start_date > n.end_date || o == this.config.last_hour) && (e.splice(a, 1), a--)
        }
    }
    if (!t) {
    ...
    for (var i, s = [], a = [
        ...
        if (l._sday = g % r, !this._ignores[l._sday] || !l._timed) {
            ...
            if (l._sorder && t || (l._sorder = v), l._sday + l._length <= r) i = null, s.push(l), m[v] = l, n[l._sweek] = m.length - 1, l._first_chunk = _.first_chunk, l._last_chunk = _.last_chunk;
                ...
                p.id = l.id, p._length = r - l._sday, p._eday = r, p._sday = l._sday, p._sweek = l._sweek, p._sorder = l._sorder, p.end_date = this.date.add(c, p._length, "day"), p._first_chunk = _.first_chunk, _.first_chunk && (_.first_chunk = !1), s.push(p), m[v] = p, i = p.end_date, n[l._sweek] = m.length - 1, o--
            }
        }
    }
    return s
    ...
    if (!(e._sday < 0)) {
        ...
        if (s) {
            ...
            if (this._rendered.push(g), s.appendChild(g), h = h + parseInt(s.style.left, 10) + i, this._edit_id == e.id) {
                ...
                this.set_xy(v, l - 6, r - 26), v.style.cssText += ";margin:2px 2px 2px 2px;overflow:hidden;", g.appendChild(v), this._els.dhx_cal_data[0].appendChild(g), this._rendered.push(g), v.innerHTML = "<textarea class='dhx_cal_editor'>" + e.text + "</textarea>", this._quirks7 && (v.firstChild.style.height = r - 12 + "px"), this._editor = v.firstChild, this._editor.onkeydown = function(e) {
    ...
    return o && (p = "dhx_resize_denied " + p), m += '<div class="' + p + '" style=" width:' + (s - 8) + "px;" + (o ? " margin-top:-1px;" : "") + u + g + '" ></div>', v.innerHTML = m, v
    ...
    parse: function(e) {
        ...
        if (t.length) {
            ...
            for (var i, s = [], a = RegExp("(?:" + this.e_start + ")([^\f]*?)(?:" + this.e_end + ")", "g"); null !== (i = a.exec(t));) {
                ...
                r.uid && !r.id && (r.id = r.uid), s.push(r)
            }
            return s
        }
    },
    ...
    select: {
        ...
        set_value: function(e, t, i, s) {
            var a = e.firstChild;
            !a._dhx_onchange && s.onchange && (a.onchange = s.onchange, a._dhx_onchange = !0), "undefined" == typeof t && (t = (a.options[0] || {}).value), a.value = t || ""
        },
        get_value: function(e) {
            return e.firstChild.value
        },
    ...
    time: {
        ...
        get_value: function(e, t, i) {
            ...
            if (t.start_date = new Date(s[a[3]].value, s[a[2]].value, s[a[1]].value, 0, s[a[0]].value), t.end_date = new Date(s[a[3] + 4].value, s[a[2] + 4].value, s[a[1] + 4].value, 0, s[a[0] + 4].value), !s[a[3]].value || !s[a[3] + 4].value) {
    ...
    if (e) {
        ...
        e.style.top = t ? Math.round(t + Math.max((s - e.offsetHeight) / 2, 0)) + "px" : Math.round(Math.max((s - e.offsetHeight) / 2, 0) + 9) + "px", e.style.left = document.documentElement.scrollWidth > document.body.offsetWidth ? Math.round(i + (document.body.offsetWidth - e.offsetWidth) / 2) + "px" : Math.round((document.body.offsetWidth - e.offsetWidth) / 2) + "px"
    }
    this.show_cover()
    ...
    if (!this._lightbox) {
        ...
        for (var s = 0; s < a.length; s++) {
            ...
            if (n) {
                ...
                "string" != typeof d && (d = a[s].name), t += "<div id='" + a[s].id + "' class='dhx_cal_lsection'>" + r + d + "</div>" + n.render.call(this, a[s]), t += "</div>"
            }
        }
    ...
    function a(e, t, i) {
        if (e && t) {
            var s = Math.abs(e.pageY - t.pageY),
                a = Math.abs(e.pageX - t.pageX);
            a > i && (!s || a / s > 3) && (e.pageX > t.pageX ? scheduler._click.dhx_cal_next_button() : scheduler._click.dhx_cal_prev_button())
        }
    }
    ...
    function n(e) {
        ...
        "create" == t && i && (scheduler.render_view_data = function() {
            ...
            t._sorder = a.length - 1, t._count = a.length, this.render_data([t], scheduler.getState().mode)
    ...
    s(document.body, e[0], function(e) {
        if (!i(e)) {
            ...
            if (o && u && a(o, t(e), 0), o = t(e), c) return o ? void((d.target != o.target || Math.abs(d.pageX - o.pageX) > 5 || Math.abs(d.pageY - o.pageY) > 5) && (_ = !0, clearTimeout(l))) : void(_ = !0)
        }
        ...
        if (!i(e)) {
            ...
            var a = new Date;
            if (!_ && !h && 250 > a - g) return scheduler._click.dhx_cal_data(s), window.setTimeout(function() {
                scheduler._on_dbl_click(s)
            }, 50), e.preventDefault && e.preventDefault(), e.cancelBubble = !0, scheduler._block_next_stop = !0, !1;
            g = a, _ || h || !scheduler.config.touch_drag || (l = setTimeout(function() {
                h = !0;
                var e = d.target;
                if (e && e.className && -1 != e.className.indexOf("dhx_body") && (e = e.previousSibling), scheduler._on_mouse_down(d, e), scheduler._drag_mode && "create" != scheduler._drag_mode) {
                    var t = -1;
                    if (scheduler.for_rendered(scheduler._drag_id, function(e, i) {
                        t = e.getBoundingClientRect().top, e.style.display = "none", scheduler._rendered.splice(i, 1)
                    }), t >= 0) {
                        var i = scheduler.config.time_step;
                        scheduler._move_pos_shift = i * Math.round(60 * (s.pageY - t) / (scheduler.config.hour_size_px * i))
                    }
                }
                scheduler.config.touch_tip && scheduler._show_global_tip(), scheduler._on_mouse_move(d)
            }, scheduler.config.touch_drag), d = s)
        }
    }), s(this._els.dhx_cal_data[0], e[2], function(e) {
        return i(e) ? void 0 : (h || a(d, o, 200), h && (scheduler._ignore_next_click = !0), r(e), scheduler._block_next_stop ? (scheduler._block_next_stop = !1, e.preventDefault && e.preventDefault(), e.cancelBubble = !0, !1) : void 0)
    ...
    if (t) {
        ...
        t.innerHTML = "create" == scheduler._drag_mode || "new-size" == scheduler._drag_mode ? (scheduler.locale.drag_to_create || "Drag to create") + i : (scheduler.locale.drag_to_move || "Drag to move") + i
    }
    ...
    e._methods = ["_set_event_text_style", "", "changeEventId", "_dp_hook_delete"], this._dp_hook_delete = function(e) {
        ...
        var t = this.obj.getEvent(e),
            i = {};
        for (var s in t) 0 !== s.indexOf("_") && (i[s] = t[s] && t[s].getUTCFullYear ? this.obj.templates.xml_format(t[s]) : t[s]);
        return i
    ...
    var s = 0;
    if (!scheduler.skin || "classic" !== scheduler.skin && "glossy" !== scheduler.skin || (s = 1), this._configure(scheduler.config, scheduler._skin_settings, s), this._configure(scheduler.xy, scheduler._skin_xy, s), "flat" === scheduler.skin && (scheduler.xy.scale_height = 35, scheduler.templates.hour_scale = function(e) {
        var t = e.getMinutes();
        t = 10 > t ? "0" + t : t;
        ...
        a && (a.padding = 14), scheduler.templates.event_bar_date = function(e) {
    ...
    function() {
        ...
        scheduler.attachEvent("onSchedulerReady", function() {
            scheduler.config.delay_render ? (scheduler.setCurrentView = n, scheduler.updateView = r) : (scheduler.setCurrentView = t, scheduler.updateView = i)
        })
    }();